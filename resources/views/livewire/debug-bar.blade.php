            ...
            <div class="ndb:relative ndb:isolate ndb:flex ndb:min-h-0 ndb:flex-1 ndb:flex-col ndb:sm:flex-row">
                ...
                <main x-ref="content" :inert="mobileSectionsOpen" class="ndb-scrollbar ndb:min-w-0 ndb:flex-1 ndb:overflow-y-auto ndb:bg-white/70 ndb:dark:bg-zinc-950/70">
                    ...
                    <div x-cloak x-show.important="detailsError" role="alert" class="ndb:m-4 ndb:rounded-xl ndb:border ndb:border-red-200 ndb:bg-red-50/70 ndb:p-4 ndb:dark:border-red-950 ndb:dark:bg-red-950/25 ndb:sm:m-6">
                        ...
                        <p class="ndb:mt-1 ndb:text-xs ndb:text-red-700/80 ndb:dark:text-red-300/80">The request summary is still available. Retry, or return to History if this profile expired.</p>
                        <button type="button" @click="requestDetails()" class="ndb:mt-3 ndb:rounded-lg ndb:bg-red-700 ndb:px-3 ndb:py-2 ndb:text-xs ndb:font-bold ndb:text-white ndb:focus-visible:outline-2 ndb:focus-visible:outline-offset-2 ndb:focus-visible:outline-red-500 ndb:dark:bg-red-300 ndb:dark:text-red-950">Retry details</button>
                    </div>
                    ...
                    @if ($detailsLoaded && $profile !== [])
                        <div wire:key="profile-details-{{ $profileId }}" wire:loading.remove wire:target="loadDetails" class="ndb:p-4 ndb:sm:p-6">
                            ...
                            <section data-ndb-section-panel="history" hidden wire:key="section-history" class="ndb:space-y-4">
                                <div><p class="ndb:text-sm ndb:font-semibold">History keeps recent requests so you can inspect background work and earlier pages.</p>@if (count($history) === 1)<p class="ndb:mt-1 ndb:text-xs ndb:text-zinc-500 ndb:dark:text-zinc-400">New requests will appear here as you use the app.</p>@endif</div>
                                ...
                                @if (! ($summary['is_current_profile'] ?? true))
                                    <div class="ndb:flex ndb:flex-wrap ndb:items-center ndb:gap-3 ndb:rounded-xl ndb:border ndb:border-indigo-200 ndb:bg-indigo-50/50 ndb:px-3.5 ndb:py-3 ndb:dark:border-indigo-950 ndb:dark:bg-indigo-950/20"><div class="ndb:min-w-0 ndb:flex-1"><p class="ndb:text-xs ndb:font-bold">Inspecting a retained profile</p><p class="ndb:mt-0.5 ndb:text-[10px] ndb:text-zinc-500 ndb:dark:text-zinc-400">The compact bar and sections show this selected request.</p></div><button type="button" data-ndb-return-current wire:click="returnToCurrent" wire:loading.attr="disabled" class="ndb:rounded-lg ndb:bg-indigo-600 ndb:px-3 ndb:py-2 ndb:text-[10px] ndb:font-bold ndb:text-white ndb:focus-visible:outline-2 ndb:focus-visible:outline-offset-2 ndb:focus-visible:outline-indigo-500 ndb:disabled:opacity-50">Back to current request</button></div>
                                @endif
                                ...
                                @if ($comparison !== [])
                                    <div data-ndb-comparison class="ndb:overflow-hidden ndb:rounded-xl ndb:border ndb:border-indigo-200/90 ndb:bg-indigo-50/30 ndb:dark:border-indigo-950 ndb:dark:bg-indigo-950/20">
                                        <div class="ndb:flex ndb:items-center ndb:gap-3 ndb:border-b ndb:border-indigo-200/70 ndb:px-4 ndb:py-3 ndb:dark:border-indigo-950"><div class="ndb:min-w-0 ndb:flex-1"><h3 class="ndb:text-xs ndb:font-bold">Comparison</h3><p class="ndb:mt-0.5 ndb:truncate ndb:text-[10px] ndb:text-zinc-500 ndb:dark:text-zinc-400">{{ $comparison['path'] }}</p><p class="ndb:mt-1 ndb:flex ndb:flex-wrap ndb:gap-x-3 ndb:gap-y-1 ndb:text-[10px] ndb:font-semibold ndb:text-zinc-400"><span>Baseline {{ $comparison['baseline']['recorded_time'] ?? 'Unknown time' }} #{{ $comparison['baseline']['short_id'] ?? 'unknown' }}</span><span>Selected {{ $comparison['current']['recorded_time'] ?? 'Unknown time' }} #{{ $comparison['current']['short_id'] ?? 'unknown' }}</span></p></div><button type="button" wire:click="clearComparison" class="ndb:text-[10px] ndb:font-bold ndb:text-indigo-700 ndb:focus-visible:outline-2 ndb:focus-visible:outline-indigo-500 ndb:dark:text-indigo-300">Clear</button></div>
                                    ...
                                    @foreach ($history as $entry)
                                        ...
                                        >
                                            ...
                                            @if (! $entry['is_selected'])
                                                <button type="button" data-ndb-open-profile="{{ $entry['id'] }}" wire:click="selectProfile('{{ $entry['id'] }}')" wire:loading.attr="disabled" aria-label="Open {{ $entry['method'] }} {{ $entry['path'] }} profile from {{ $entry['recorded_time'] ?? 'unknown time' }}" class="ndb:self-start ndb:rounded-lg ndb:bg-zinc-900 ndb:px-3 ndb:py-2 ndb:text-[10px] ndb:font-bold ndb:text-white ndb:transition ndb:hover:bg-indigo-700 ndb:focus-visible:outline-2 ndb:focus-visible:outline-offset-2 ndb:focus-visible:outline-indigo-500 ndb:disabled:opacity-50 ndb:sm:self-center ndb:dark:bg-zinc-100 ndb:dark:text-zinc-900 ndb:dark:hover:bg-indigo-300">Open</button>
                                            @endif
                                            @if ($entry['comparable'])
