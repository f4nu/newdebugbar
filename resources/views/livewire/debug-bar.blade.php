                            ...
                            <section data-ndb-section-panel="history" hidden wire:key="section-history" class="ndb:space-y-4">
                                ...
                                <div x-ref="historyList" x-init="$nextTick(() => applyHistoryFilters())" class="ndb:space-y-2">
                                    @foreach ($history as $entry)
                                        ...
                                        >
                                            <div class="ndb:min-w-0 ndb:flex-1"><div class="ndb:flex ndb:items-center ndb:gap-2"><span class="ndb:rounded-md ndb:bg-indigo-50 ndb:px-1.5 ndb:py-0.5 ndb:text-[9px] ndb:font-bold ndb:text-indigo-700 ndb:dark:bg-indigo-950 ndb:dark:text-indigo-300">{{ $entry['method'] }}</span><p class="ndb:truncate ndb:text-xs ndb:font-bold">{{ $entry['path'] }}</p>@if ($entry['is_current'])<span class="ndb:text-[9px] ndb:font-bold ndb:text-sky-700 ndb:dark:text-sky-300">Current</span>@endif @if ($entry['is_selected'])<span class="ndb:text-[9px] ndb:font-bold ndb:text-indigo-700 ndb:dark:text-indigo-300">Selected</span>@endif</div><div class="ndb:mt-1.5 ndb:flex ndb:flex-wrap ndb:gap-x-4 ndb:gap-y-1 ndb:text-[10px] ndb:font-semibold ndb:text-zinc-400"><span>{{ $entry['recorded_time'] ?? 'Unknown time' }}</span><span>#{{ $entry['short_id'] ?? 'unknown' }}</span><span>{{ $entry['status'] }}</span><span>{{ str($entry['request_type'])->replace('_', ' ')->title() }}</span><span>{{ $entry['duration_ms'] }} ms</span><span>{{ $entry['query_count'] }} queries</span><span>{{ $entry['finding_count'] }} findings</span></div></div>
