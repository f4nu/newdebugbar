                                ...
                                >
                                    ...
                                    @elseif ($sectionKey === 'request')
                                        <div class="ndb:grid ndb:grid-cols-2 ndb:gap-3 ndb:lg:grid-cols-4">
                                            @foreach (['method' => 'Method', 'status' => 'Status', 'route' => 'Route', 'action' => 'Controller'] as $key => $label)
                                                <div class="ndb:min-w-0 ndb:rounded-xl ndb:border ndb:border-zinc-200 ndb:p-3 ndb:dark:border-zinc-800">
                                                    <p class="ndb:text-[10px] ndb:font-semibold ndb:uppercase ndb:tracking-wider ndb:text-zinc-400">
                                                        {{ $label }}
                                                    </p>
                                                    <p class="ndb:mt-1 ndb:truncate ndb:text-xs ndb:font-semibold">
                                                        {{ $section['payload'][$key] ?: '—' }}
                                                    </p>
                                                </div>
                                            @endforeach
                                        ...
                                        <dl class="ndb:grid ndb:grid-cols-2 ndb:divide-x ndb:divide-y ndb:divide-zinc-200 ndb:overflow-hidden ndb:rounded-xl ndb:border ndb:border-zinc-200 ndb:lg:grid-cols-5 ndb:lg:divide-y-0 ndb:dark:divide-zinc-800 ndb:dark:border-zinc-800">
                                            @foreach ([
                                                ...
                                                <div class="ndb:min-w-0 ndb:px-3 ndb:py-2.5">
                                                    ...
                                                    <dd class="ndb:mt-0.5 ndb:truncate ndb:text-xs ndb:font-bold">
                                                        {{ $value }}
                                                    </dd>
                                                </div>
                                            @endforeach
                                        ...
                                        <div class="ndb:grid ndb:gap-3 ndb:lg:grid-cols-2">
                                            ...
                                            <div class="ndb:rounded-xl ndb:border ndb:border-zinc-200 ndb:p-4 ndb:dark:border-zinc-800">
                                                ...
                                                <dl class="ndb:mt-3 ndb:grid ndb:grid-cols-2 ndb:gap-3">
                                                    <div>
                                                        <dt class="ndb:text-[9px] ndb:font-semibold ndb:text-zinc-400">
                                                            Guard
                                                        </dt>
                                                        <dd class="ndb:mt-0.5 ndb:text-xs ndb:font-bold">
                                                            {{ $authentication['guard'] ?? 'unknown' }}
                                                        </dd>
                                                    </div>
                                                    <div>
                                                        <dt class="ndb:text-[9px] ndb:font-semibold ndb:text-zinc-400">
                                                        ...
                                                        <dd class="ndb:mt-0.5 ndb:truncate ndb:text-xs ndb:font-bold">
                                                            {{ $authentication['model'] ?? 'Guest' }}
                                                        </dd>
                                                    </div>
                                                    @if (is_string($authentication['identifier'] ?? null))
                                                        <div class="ndb:col-span-2">
                                                            <dt class="ndb:text-[9px] ndb:font-semibold ndb:text-zinc-400">
                                                                Private identifier
                                                            </dt>
                                                            <dd class="ndb:mt-0.5 ndb:truncate ndb:font-mono ndb:text-[10px]">
                                            ...
                                            <div class="ndb:rounded-xl ndb:border ndb:border-zinc-200 ndb:p-4 ndb:dark:border-zinc-800">
                                                ...
                                                @if ($sessionShape['present'] ?? false)
                                                    <p class="ndb:mt-2 ndb:flex ndb:flex-wrap ndb:gap-x-3 ndb:gap-y-1 ndb:text-xs ndb:font-semibold">
                                                        <span>{{ $sessionShape['key_count'] }} keys</span
                                                        ><span>{{ $sessionShape['driver'] ?? 'unknown' }} driver</span>
                                                    </p>
                                                @else
                                                    <p class="ndb:mt-2 ndb:text-xs ndb:font-semibold">
                                                        No started session
                                                    </p>
                                                @endif
                                                @if (($sessionShape['keys'] ?? []) !== [])
                                                    <p class="ndb:mt-2 ndb:break-words ndb:text-[10px] ndb:text-zinc-500 ndb:dark:text-zinc-400">
                                                        {{ implode(', ', $sessionShape['keys']) }}
                                                    </p>
                                                @endif
                                                @if (($sessionShape['flash_keys'] ?? []) !== [] || ($sessionShape['error_bags'] ?? []) !== [])
                                                    <p class="ndb:mt-2 ndb:flex ndb:flex-wrap ndb:gap-x-3 ndb:gap-y-1 ndb:text-[10px] ndb:font-semibold ndb:text-zinc-500 ndb:dark:text-zinc-400">
                                                        <span
                                                            >Flash: {{ implode(', ', $sessionShape['flash_keys'] ?? []) ?: 'none' }}</span
                                                        ><span
                                                            >Error bags: {{ implode(', ', $sessionShape['error_bags'] ?? []) ?: 'none' }}</span>
                                                    </p>
                                                @endif
                                            </div>
                                        </div>
                                        <details class="ndb:group ndb:overflow-hidden ndb:rounded-xl ndb:border ndb:border-zinc-200 ndb:dark:border-zinc-800">
                                            <summary class="ndb:flex ndb:cursor-pointer ndb:list-none ndb:items-center ndb:gap-3 ndb:px-4 ndb:py-3 ndb:focus-visible:outline-2 ndb:focus-visible:outline-indigo-500">
                                                <span class="ndb:min-w-0 ndb:flex-1"
                                                    ><span class="ndb:block ndb:text-xs ndb:font-bold"
                                                        >Middleware pipeline</span
                                                    ><span
                                                        class="ndb:mt-0.5 ndb:block ndb:text-[10px] ndb:text-zinc-400"
                                                        >{{ count($section['payload']['middleware'] ?? []) }} configured
                                                        steps</span
                                                    ></span
                                                ><x-newdebugbar::icon
                                                    name="chevron-down"
                                            ...
                                            <ol class="ndb:list-none ndb:space-y-2 ndb:border-t ndb:border-zinc-200 ndb:p-4 ndb:dark:border-zinc-800">
                                                @forelse ($section['payload']['middleware'] ?? [] as $position => $middleware)
                                                    <li class="ndb:flex ndb:min-w-0 ndb:items-center ndb:gap-3 ndb:text-xs">
                                                        <span
                                                            class="ndb:w-5 ndb:shrink-0 ndb:text-right ndb:font-bold ndb:tabular-nums ndb:text-zinc-400"
                                                            >{{ $position + 1 }}</span
                                                        ><code class="ndb:truncate">{{ $middleware }}</code>
                                                    </li>
                                                @empty
                                                    <li class="ndb:text-xs ndb:text-zinc-400">
                                                        No route middleware was resolved.
                                                    </li>
                                                @endforelse
                                            </ol>
                                        </details>
                                        <details class="ndb:group ndb:overflow-hidden ndb:rounded-xl ndb:border ndb:border-zinc-200 ndb:dark:border-zinc-800">
                                            <summary class="ndb:flex ndb:cursor-pointer ndb:list-none ndb:items-center ndb:gap-3 ndb:px-4 ndb:py-3 ndb:focus-visible:outline-2 ndb:focus-visible:outline-indigo-500">
                                                <span class="ndb:min-w-0 ndb:flex-1"
                                                    ><span class="ndb:block ndb:text-xs ndb:font-bold"
                                                        >Raw request data</span
                                                    ><span
                                                        class="ndb:mt-0.5 ndb:block ndb:text-[10px] ndb:text-zinc-400"
                                                        >Headers, inputs, session shape, and internal capture
                                                        fields</span
                                                    ></span
                                                ><x-newdebugbar::icon
                                                    name="chevron-down"
                                            ...
                                            <pre class="ndb-code ndb-scrollbar ndb:rounded-none ndb:border-t ndb:border-zinc-200 ndb:dark:border-zinc-800"><code data-ndb-language="json">{{ json_encode($section['payload'], JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES) }}</code></pre>
                                        </details>
                                    @elseif ($sectionKey === 'queries')
                                                ...
                                                @foreach ([
                                                    ...
                                                    <div class="ndb:px-3 ndb:py-2.5">
                                                        ...
                                                        <dd
                                                            ...
                                                            class="ndb:mt-0.5 ndb:text-sm ndb:font-bold ndb:tabular-nums"
                                                        >
                                                            {{ $value }}
                                                        </dd>
                                                    </div>
                                        ...
                                        <div class="ndb:flex ndb:flex-col ndb:gap-3 ndb:border-b ndb:border-zinc-200/80 ndb:pb-3 ndb:lg:flex-row ndb:lg:items-end ndb:dark:border-zinc-800">
                                                ...
                                                >
                                                    @foreach (['all' => 'All', 'repeated' => 'Repeated', 'slow' => 'Slow', 'read' => 'Read', 'write' => 'Write'] as $filter => $label)
                                                        <button
                                                            ...
                                                            class="ndb:rounded-lg ndb:border ndb:px-3 ndb:py-1.5 ndb:text-xs ndb:font-semibold ndb:transition ndb:focus-visible:outline-2 ndb:focus-visible:outline-indigo-500"
                                                            :class="queryFilter === @js($filter) ? 'ndb:border-indigo-200 ndb:bg-indigo-50 ndb:text-indigo-700 ndb:dark:border-indigo-900 ndb:dark:bg-indigo-950/60 ndb:dark:text-indigo-300' : 'ndb:border-transparent ndb:text-zinc-500 ndb:hover:border-zinc-200 ndb:hover:bg-white/70 ndb:hover:text-zinc-950 ndb:dark:text-zinc-400 ndb:dark:hover:border-zinc-700 ndb:dark:hover:bg-zinc-900/70 ndb:dark:hover:text-white'"
                                                        >
                                                            {{ $label }}
                                                        </button>
                                                    @endforeach
                                            ...
                                            <label class="ndb:min-w-0 ndb:lg:w-64"
                                                ><span
                                                    class="ndb:mb-1.5 ndb:block ndb:text-[9px] ndb:font-semibold ndb:uppercase ndb:tracking-wider ndb:text-zinc-400"
                                                    >Search</span
                                                ><input
                                                    ...
                                                    type="search"
                                                    placeholder="SQL or redacted binding"
                                                    class="ndb:h-9 ndb:w-full ndb:rounded-lg ndb:border ndb:border-zinc-200 ndb:bg-white/70 ndb:px-3 ndb:text-xs ndb:outline-none ndb:transition ndb:placeholder:text-zinc-400 ndb:focus:border-indigo-400 ndb:focus:ring-2 ndb:focus:ring-indigo-500/15 ndb:dark:border-zinc-700 ndb:dark:bg-zinc-900/70"
                                            /></label>
                                            <div>
                                                <p class="ndb:mb-1.5 ndb:text-[9px] ndb:font-semibold ndb:uppercase ndb:tracking-wider ndb:text-zinc-400">
                                                    Sort
                                                ...
                                                <div
                                                    class="ndb:flex ndb:rounded-lg ndb:bg-zinc-200/70 ndb:p-0.5 ndb:dark:bg-zinc-800/80"
                                                    role="group"
                                                ...
                                                >
                                                    @foreach (['execution' => 'Execution', 'duration' => 'Slowest'] as $sort => $label)
                                                        <button
                                                            type="button"
                                                            data-ndb-query-sort="{{ $sort }}"
                                                            @click="setQuerySort(@js($sort))"
                                                            :aria-pressed="querySort === @js($sort)"
                                                            class="ndb:flex ndb:h-8 ndb:min-w-20 ndb:items-center ndb:justify-center ndb:rounded-md ndb:px-3 ndb:text-xs ndb:font-bold ndb:transition ndb:focus-visible:outline-2 ndb:focus-visible:outline-indigo-500"
                                                            :class="querySort === @js($sort) ? 'ndb:bg-white ndb:text-zinc-950 ndb:shadow-sm ndb:dark:bg-zinc-700 ndb:dark:text-white' : 'ndb:text-zinc-500 ndb:hover:text-zinc-950 ndb:dark:text-zinc-400 ndb:dark:hover:text-white'"
                                                        >
                                                            {{ $label }}
                                                        </button>
                                                    @endforeach
                                        ...
                                        >
                                            <span x-text="visibleQueryCount"></span> results
                                        </p>
                                                ...
                                                >
                                                    <div class="ndb:bg-gradient-to-br ndb:from-violet-50 ndb:via-white ndb:to-rose-50/70 ndb:p-4 ndb:dark:from-violet-950/45 ndb:dark:via-zinc-950 ndb:dark:to-rose-950/25">
                                                        <div class="ndb:flex ndb:items-start ndb:gap-3">
                                                            <span
                                                                class="ndb:grid ndb:size-10 ndb:shrink-0 ndb:place-items-center ndb:rounded-xl ndb:bg-violet-100 ndb:text-violet-700 ndb:dark:bg-violet-900/60 ndb:dark:text-violet-300"
                                                                ><x-newdebugbar::icon
                                                                    name="database"
                                                                    class="ndb:size-5"
                                                            /></span>
                                                            <div class="ndb:min-w-0 ndb:flex-1">
                                                                <h3
                                                                    ...
                                                                    class="ndb:text-sm ndb:font-bold"
                                                                >
                                                                    {{ $group['count'] }} matching executions
                                                                </h3>
                                                                <p
                                                                    data-ndb-query-group-extra
                                                                    class="ndb:mt-0.5 ndb:text-xs ndb:text-zinc-600 ndb:dark:text-zinc-300"
                                                                >
                                                                    {{ $group['extra_executions'] }} extra {{ $group['extra_executions'] === 1 ? 'execution' : 'executions' }} from
                                                                    one query pattern
                                                                </p>
                                                            </div>
                                                            <div class="ndb:shrink-0 ndb:text-right">
                                                                <p
                                                                    ...
                                                                    class="ndb:text-lg ndb:font-bold ndb:tabular-nums"
                                                                >
                                                                    {{ $group['duration_ms'] }} ms
                                                                </p>
                                                                <p class="ndb:text-[10px] ndb:font-semibold ndb:text-zinc-500 ndb:dark:text-zinc-400">
                                                                    Total query time
                                                                </p>
                                                            </div>
                                                        </div>
                                                    ...
                                                    <div
                                                        ...
                                                        class="ndb:border-y ndb:border-zinc-200/80 ndb:bg-zinc-50/70 ndb:dark:border-zinc-800 ndb:dark:bg-zinc-900/70"
                                                    >
                                                        <div class="ndb:flex ndb:items-center ndb:justify-between ndb:gap-3 ndb:px-4 ndb:py-2 ndb:text-zinc-600 ndb:dark:text-zinc-300">
                                                            <span class="ndb:text-[10px] ndb:font-bold"
                                                                >Query pattern</span
                                                            ><button
                                                                type="button"
                                                                @click="copyText(@js($group['sql']))"
                                                                class="ndb:inline-flex ndb:size-7 ndb:items-center ndb:justify-center ndb:rounded-lg ndb:text-zinc-500 ndb:transition ndb:hover:bg-zinc-200/70 ndb:hover:text-zinc-950 ndb:focus-visible:outline-2 ndb:focus-visible:outline-violet-400 ndb:dark:text-zinc-400 ndb:dark:hover:bg-zinc-800 ndb:dark:hover:text-white"
                                                                aria-label="Copy repeated query pattern"
                                                                title="Copy query pattern"
                                                            >
                                                    ...
                                                    <div data-ndb-query-group-executions class="ndb:space-y-2 ndb:p-3">
                                                        <div class="ndb:flex ndb:items-center ndb:justify-between ndb:gap-3 ndb:px-1">
                                                            <h4 class="ndb:text-xs ndb:font-bold">Executions</h4>
                                                            <span class="ndb:text-[10px] ndb:font-semibold ndb:text-zinc-400">Ordered by the selected sort</span>
                                                        </div>
