...
>
  ...
  >
    <div
      class="ndb:border-b ndb:border-zinc-200/90 ndb:bg-zinc-50/65 ndb:px-4 ndb:py-3 ndb:dark:border-zinc-800 ndb:dark:bg-zinc-900/45"
    >
    ...
    <div
      x-show.important="filteredLivewireComponents.length === 0"
      class="ndb:p-4"
    >
      <x-newdebugbar::empty-state
        label="No mounted components match this search."
      />
    </div>
  </div>
  ...
  >
      ...
      <article class="ndb:min-w-0">
        ...
        >
          <div class="ndb:flex ndb:min-w-0 ndb:items-start ndb:gap-4">
            <span
              class="ndb:mt-0.5 ndb:grid ndb:size-8 ndb:shrink-0 ndb:place-items-center ndb:rounded-lg ndb:bg-indigo-50 ndb:text-indigo-600 ndb:dark:bg-indigo-950/60 ndb:dark:text-indigo-300"
            >
          ...
          <dl
            class="ndb:mt-4 ndb:grid ndb:grid-cols-2 ndb:gap-x-4 ndb:gap-y-3 ndb:sm:grid-cols-4"
          >
            <div class="ndb:min-w-0">
              ...
              <dd
                class="ndb:mt-1 ndb:flex ndb:min-w-0 ndb:items-center ndb:gap-1.5"
              >
                <code
            ...
            <div class="ndb:min-w-0">
              ...
              <dd
                ...
                x-text="
                  selectedLivewireComponent.server?.view?.name ?? 'Not exposed'
                "
              ></dd>
            </div>
          </dl>
          ...
          <section>
            ...
            >
              <div>
                ...
                <p class="ndb:mt-0.5 ndb:text-[11px] ndb:text-zinc-400">Current browser state with the latest server-confirmed value.</p>
              </div>
              <span
                class="ndb:text-[11px] ndb:font-semibold ndb:tabular-nums ndb:text-zinc-400"
                x-text="`${selectedLivewireComponent.properties.length} public`"
              ></span>
            </div>
            ...
            >
              ...
              >
                ...
                >
                  <div
                    class="ndb:grid ndb:min-w-0 ndb:gap-2 ndb:px-3 ndb:py-2.5 ndb:sm:grid-cols-[minmax(10rem,1fr)_minmax(7rem,0.8fr)_minmax(7rem,0.8fr)_5rem_3rem] ndb:sm:items-center ndb:sm:gap-3"
                  >
                    <div
                    ...
                    <div
                      class="ndb:flex ndb:min-w-0 ndb:items-baseline ndb:gap-2 ndb:sm:block"
                    >
                      <span
                        class="ndb:w-16 ndb:shrink-0 ndb:text-[11px] ndb:font-semibold ndb:uppercase ndb:tracking-wider ndb:text-zinc-400 ndb:sm:hidden"
                        >Browser</span
                      >
                      <code
            ...
            >
              <x-newdebugbar::empty-state
                label="This component has no serialized public properties."
              />
            </div>
          </section>
          ...
          <section>
            ...
            <div class="ndb:mt-3 ndb:space-y-2">
              <template
                x-for="
                  item in
                  livewireComponentActivity(selectedLivewireComponent.id)
                "
                :key="item.id"
              >
                <button
                  type="button"
                  @click="
                    livewireSelectedActivityId = item.id;
                    livewireTab = 'activity';
                    livewireDetailOpen = true;
                  "
              ...
              <p x-show.important="livewireComponentActivity(selectedLivewireComponent.id).length === 0" class="ndb:text-xs ndb:text-zinc-400">No activity has been observed for this instance.</p>
            </div>
          </section>
        </div>
      </article>
    ...
    <div x-show.important="!selectedLivewireComponent" class="ndb:p-5">
      <x-newdebugbar::empty-state
        label="Select a mounted component to inspect it."
      />
    </div>
  </div>
</div>
