...
<div data-ndb-models class="ndb:space-y-5 ndb:text-zinc-950 ndb:dark:text-white">
    @if ($modelGroups !== [])
        ...
        >
            <dl class="ndb:grid ndb:grid-cols-2 ndb:sm:grid-cols-4">
                @foreach ([
                    ...
                    <div @class([
                        ...
                        <dd class="ndb:mt-1 ndb:text-lg ndb:font-bold ndb:leading-none ndb:tabular-nums ndb:text-zinc-950 ndb:dark:text-white">
                            {{ number_format($value) }}
                        </dd>
            ...
            <div class="ndb:border-t ndb:border-zinc-200/90 ndb:px-3 ndb:py-2.5 ndb:text-[11px] ndb:leading-5 ndb:text-zinc-500 ndb:dark:border-zinc-800 ndb:dark:text-zinc-400">
                <p>
                    Retrieved instances count Eloquent
                    <code class="ndb:font-mono ndb:font-semibold">retrieved</code> events. They are not query counts or
                    database row counts. One query can retrieve many model instances.
                </p>
                @if ($intermediateCount > 0)
                    <p class="ndb:mt-1">
                        {{ number_format($lifecycleCount) }} lifecycle callbacks were captured. {{ number_format($intermediateCount) }} intermediate {{ \Illuminate\Support\Str::plural('callback', $intermediateCount) }} were
        ...
        @if ($modelContextCount > $modelClassCount)
            ...
            >
                ...
                in {{ number_format($modelContextCount) }} connection or table contexts. Contexts stay separate so their
                counts are not mixed.
            </p>
        @endif
        ...
        @if ($unknownSourceCount > 0)
            ...
            >
                The application source was unavailable for {{ number_format($unknownSourceCount) }} model {{ \Illuminate\Support\Str::plural('activity', $unknownSourceCount) }}.
                Counts remain complete, but that activity cannot be traced to an app file.
            </p>
        @endif
            ...
            <div class="ndb:flex ndb:items-end ndb:justify-between ndb:gap-3">
                <div>
                    ...
                    <p class="ndb:mt-0.5 ndb:text-[11px] ndb:text-zinc-500 ndb:dark:text-zinc-400">
                        Writes appear first, followed by classes with repeated retrievals.
                    </p>
                </div>
