                ...
                @foreach ($livewire['components'] as $component)
                    ...
                    >
                        <header class="ndb:min-w-0">
                            <h3 class="ndb:truncate ndb:text-lg ndb:font-bold ndb:tracking-tight">{{ $component['display_name'] }}</h3>
                            <p class="ndb:mt-1 ndb:truncate ndb:text-[10px] ndb:text-zinc-400">{{ $component['raw_name'] }}</p>
                        </header>
                        ...
                        <dl class="ndb:grid ndb:grid-cols-2 ndb:gap-x-4 ndb:gap-y-3 ndb:border-y ndb:border-zinc-200 ndb:py-3 ndb:text-xs ndb:sm:grid-cols-3 ndb:dark:border-zinc-800">
                            <div>
                                <dt class="ndb:text-[9px] ndb:font-semibold ndb:uppercase ndb:tracking-wider ndb:text-zinc-400">Trigger</dt>
                                <dd class="ndb:mt-1 ndb:font-bold">{{ $component['trigger_label'] }}</dd>
                            </div>
                            <div>
                                <dt class="ndb:text-[9px] ndb:font-semibold ndb:uppercase ndb:tracking-wider ndb:text-zinc-400">Result</dt>
                                <dd class="ndb:mt-1 ndb:font-bold">{{ $component['result_label'] }}</dd>
                            </div>
                            <div>
                                <dt class="ndb:text-[9px] ndb:font-semibold ndb:uppercase ndb:tracking-wider ndb:text-zinc-400">Render</dt>
                                <dd class="ndb:mt-1 ndb:font-bold">{{ $component['rendered_label'] }}</dd>
                            </div>
                        </dl>

                        @if ($component['redirect'] !== null || $component['download'] !== null)
                            <section data-ndb-livewire-response-effect class="ndb:border-l-2 ndb:border-indigo-300 ndb:pl-3 ndb:dark:border-indigo-800">
                                <h4 class="ndb:text-xs ndb:font-bold">Response</h4>
                                @if ($component['redirect'] !== null)
                                    <p class="ndb:mt-1 ndb:break-all ndb:text-xs ndb:text-zinc-600 ndb:dark:text-zinc-300">Redirect: {{ $component['redirect'] }}</p>
                                @endif
                                @if ($component['download'] !== null)
                                    <p class="ndb:mt-1 ndb:break-all ndb:text-xs ndb:text-zinc-600 ndb:dark:text-zinc-300">Download: {{ $component['download']['name'] }}</p>
                                @endif
                            </section>
                        @endif
                                    ...
                                    @foreach ($component['state_changes'] as $change)
                                        <div class="ndb:grid ndb:gap-2 ndb:border-b ndb:border-zinc-200 ndb:py-2.5 ndb:sm:grid-cols-[minmax(7rem,0.5fr)_minmax(0,1.5fr)] ndb:sm:items-center ndb:dark:border-zinc-800">
                                            ...
                                            <div class="ndb:grid ndb:min-w-0 ndb:grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] ndb:items-center ndb:gap-2 ndb:text-xs">
                                                ...
                                                <strong class="ndb:truncate" title="{{ $change['after_display'] }}">{{ $change['after_display'] }}</strong>
                                            </div>
                                        </div>
                                    @endforeach
                                </div>
                        ...
                        @if ($component['validation_fields'] !== [])
                            <section class="ndb:border-l-2 ndb:border-amber-400 ndb:pl-3">
                                ...
                                <p class="ndb:mt-1 ndb:text-xs ndb:text-zinc-600 ndb:dark:text-zinc-300">Fields with errors: {{ implode(', ', $component['validation_fields']) }}</p>
                            </section>
                        @endif
                        ...
                        @if ($component['emitted_events'] !== [] || $component['received_events'] !== [])
                            <section>
                                ...
                                <div class="ndb:mt-2 ndb:grid ndb:gap-3 ndb:sm:grid-cols-2">
                                    <div>
                                        <p class="ndb:text-[9px] ndb:font-semibold ndb:uppercase ndb:tracking-wider ndb:text-zinc-400">Emitted</p>
                                        <p class="ndb:mt-1 ndb:text-xs ndb:leading-5">{{ $component['emitted_events'] === [] ? 'None observed' : implode(', ', array_column($component['emitted_events'], 'display_name')) }}</p>
                                    </div>
                                    <div>
                                        <p class="ndb:text-[9px] ndb:font-semibold ndb:uppercase ndb:tracking-wider ndb:text-zinc-400">Received</p>
                                        <p class="ndb:mt-1 ndb:text-xs ndb:leading-5">{{ $component['received_events'] === [] ? 'None observed' : implode(', ', array_column($component['received_events'], 'display_name')) }}</p>
                                    </div>
                                </div>
                            </section>
                        @endif
                        ...
                        <section data-ndb-livewire-component-details class="ndb:border-t ndb:border-zinc-200 ndb:pt-4 ndb:dark:border-zinc-800">
                            ...
                            <dl class="ndb:mt-3 ndb:grid ndb:gap-3 ndb:text-[10px] ndb:sm:grid-cols-2">
                                @foreach (['Source' => $component['source_label'], 'Class' => $component['class'], 'View' => $component['view_label'], 'Instance' => $component['id']] as $label => $value)
                                    <div class="ndb:min-w-0">
                                        ...
                                        <dd class="ndb:mt-0.5 ndb:break-all ndb:font-semibold">{{ $value ?? 'Not observed' }}</dd>
                                    </div>
                                @endforeach
                            </dl>
                        </section>
