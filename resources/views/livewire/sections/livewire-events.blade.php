            ...
            >
                @foreach ($livewire['events'] as $event)
                    <li>
                        ...
                        >
                            <span class="ndb:shrink-0 ndb:text-[11px] ndb:font-bold ndb:tabular-nums ndb:text-zinc-400">{{ $event['sequence'] }}</span>
                            <span class="ndb:min-w-0">
                                <span class="ndb:block ndb:truncate ndb:text-xs ndb:font-bold">{{ $event['display_name'] }}</span>
                                <span class="ndb:mt-0.5 ndb:block ndb:truncate ndb:text-[11px] ndb:text-zinc-400">{{ $event['source_name'] ?? 'Source not observed' }}</span>
                            </span>
                        </button>
                    </li>
                @endforeach
                ...
                @foreach ($livewire['events'] as $event)
                    ...
                    >
                        <header class="ndb:min-w-0">
                            ...
                            <code class="ndb:mt-1 ndb:block ndb:truncate ndb:text-[11px] ndb:text-zinc-400">{{ $event['name'] }}</code>
                        </header>
