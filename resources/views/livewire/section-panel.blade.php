...
@if ($profile !== [] && is_array($section))
    <div
        ...
        :class="['cache', 'http_client', 'mail', 'notifications'].includes(selected) ? 'ndb:lg:min-h-0 ndb:lg:flex-1' : ''"
        class="ndb:p-4 ndb:sm:p-6"
    >
        <section
            data-ndb-section-panel="{{ $sectionKey }}"
            @class([
                'ndb:space-y-4' => ! in_array($sectionKey, ['cache', 'http_client', 'mail', 'notifications'], true),
                'ndb:space-y-4 ndb:lg:flex ndb:lg:h-full ndb:lg:min-h-0 ndb:lg:flex-col ndb:lg:gap-4 ndb:lg:space-y-0' => in_array($sectionKey, ['cache', 'http_client', 'mail', 'notifications'], true),
            ])
        >
