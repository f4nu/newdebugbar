...
it('groups queued channel facts by their dispatch correlation key', function () {
    $collector = new NotificationCollector(new Redactor, 10);

    foreach (['mail', 'database'] as $channel) {
        $collector->record([
            'status' => 'queued',
            'correlation_key' => 'database|notifications|job-123',
            'notification' => 'App\\Notifications\\QueuedJourneyReady',
            'channel' => $channel,
        ]);
    }
