...
| Server span | Internal debug-only `profile` event through the gateway | `app.debug=true` and contract available | Server monotonic range | Phase, component, and duration only | Bounded spans | Missing span with `unknown` duration |
