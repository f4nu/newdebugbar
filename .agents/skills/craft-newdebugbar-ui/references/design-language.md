...
- Queries EXPLAIN failures: when a safe classification is available, state it with a concrete next step. Otherwise say that the separate request failed and point to a safe next check. Never expose raw exception text, SQL bindings, database paths, or connection details.
