...
The expanded inspector shows only request-relevant sections: populated collectors, findings, truncation, incomplete data, favorites, and the current section stay visible. Favorites stay first in the order they were added and can be reordered. Overview follows Favorites unless it is a favorite itself, and every remaining section is alphabetical. The command palette always searches every section, including quiet collectors. The compact bottom toolbar width-limits the request on narrow screens, keeps its fact buttons in a horizontal scroll rail, and pins search and expand to the right. The inspector header uses the same treatment and keeps request, environment, duration, peak memory, and query facts visible without separator glyphs. On narrow screens, a fixed frameless icon control opens the same navigation as a full-surface sliding left drawer and becomes its close control while the quick facts remain in a roomy horizontal scroll rail. The exposed tap-to-dismiss area stays transparent. Overview leads with findings and relevant activity; Runtime, drivers, framework cache, and Ecosystem remain available in one secondary disclosure.
